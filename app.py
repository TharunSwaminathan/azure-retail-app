...
# This is the Flask application that serves as the backend for Retail Analytics.
# It connects to the SQL Server database and renders HTML templates for the user interface.
...
db_password = 'Retail@123'
db_name = 'RetailDataDB'
...
def query_customer_trends(query, data=None, attempt=1):
    try:
        ...
        # Commit for non-query operations
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.rollback()
            return render_template('register.html', error="An error occurred while creating the account. Please try again.")
...
def generate_table(hshd_num):
    query_string = """
    SELECT Transactions3.HSHD_NUM, BASKET_NUM, PURCHASE_, Products.PRODUCT_NUM, DEPARTMENT, COMMODITY, SPEND,
    UNITS, STORE_R, WEEK_NUM, YEAR, L, AGE_RANGE, MARITAL, INCOME_RANGE, HOMEOWNER, HSHD_COMPOSITION, HH_SIZE, CHILDREN
    FROM ((Transactions3 JOIN Households5 ON Transactions3.HSHD_NUM = Households5.HSHD_NUM) 
    JOIN Products ON Transactions3.PRODUCT_NUM = Products.PRODUCT_NUM)
    WHERE Transactions3.HSHD_NUM = ?
    ORDER BY Transactions3.HSHD_NUM, BASKET_NUM, PURCHASE_, Products.PRODUCT_NUM, DEPARTMENT, COMMODITY
    """
